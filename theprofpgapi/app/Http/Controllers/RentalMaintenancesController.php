...
class RentalMaintenancesController extends Controller
{
    // public function __construct(){
    //     $this->middleware('jwt.auth');
    // }
    private $title = "Rental Maintenance";
    ...
    public function index() {
        $data = RentalMaintenance::orderBy('title')->get();
        return Response::json([
            'data' => $this->transformCollection($data)
        ], 200);
    }
    public function show($id) {
        $data = RentalMaintenance::find($id);
    ...
    public function store(Request $request) {
        ...
        $data = RentalMaintenance::create($request->all());
        return Response::json([
